...
def more_boar(player_score, opponent_score):
    ...
    opponent_frist, opponent_second = apart_left_digit(opponent_score)
    player_first, player_second = apart_left_digit(player_score)
    return player_first < opponent_frist and player_second < opponent_second
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        current, opponent, strategy = (
            score1, score0, strategy1) if who else (score0, score1, strategy0)
        first = True
        while (first or more_boar(current, opponent)) and current < goal:
            first = False
            current += take_turn(strategy(current, opponent),
                                 opponent, dice, goal)
            score0, score1 = (opponent, current) if who else (
                current, opponent)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num, result_num, max_average = 1, 0, 0
    averaged_roll_dice = make_averaged(roll_dice, trials_count)
    while num <= 10:
        average = averaged_roll_dice(num, dice)
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    trigger_more_boar = more_boar(
        score + piggy_points(opponent_score), opponent_score)
    trigger_piggypoints = piggypoints_strategy(
        score, opponent_score, cutoff, num_rolls) == 0
    return 0 if trigger_more_boar or trigger_piggypoints else num_rolls
    # END PROBLEM 11
